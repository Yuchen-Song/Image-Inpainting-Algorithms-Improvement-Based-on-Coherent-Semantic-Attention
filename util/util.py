...
def cal_feat_mask(inMask, conv_layers, threshold):
    ...
    if inMask.is_cuda:
        lnet = lnet.cuda()
    output = lnet(inMask)
    output_float=output.clone()###
    output_float=(output > threshold).float().mul_(output)###
    output = (output > threshold).float().mul_(1)
    output=Variable(output, requires_grad = False)
    output_float=Variable(output_float, requires_grad = False)###不知道 当加不当加？
    return output.detach().byte(),output_float.detach()   #32*32 的mask 生成的地方 ###这里得改
...
def cal_mask_given_mask_thred(img, mask, patch_size, stride, mask_thred):
    ...
    for i in range(N):
        ...
        #mask__float_tmp=mask_float[h*stride:h*stride + patch_size,
         #               w*stride:w*stride + patch_size]###
        ...
        if torch.sum(mask_tmp) < mask_thred:
            ...
            tmp_mask_idx += 1
            flag[i] = 1#损坏的点位置 flag为1
            offsets_tmp_vec[i] = -1#损坏的点位置 offsets为-1
            
            ###mask_thred==1 的情况
        #if torch.sum(mask_float_tmp) > 0:
            #mask_float[h,w] = 
    ...
    for i in range(N):
        offset_value = torch.sum(offsets_tmp_vec[0:i+1])#储存的是 到这个点之前前面的所有损坏点的数量
        if flag[i] == 1:
            offset_value = offset_value + 1#如果这个点为坏的左移位置减少
