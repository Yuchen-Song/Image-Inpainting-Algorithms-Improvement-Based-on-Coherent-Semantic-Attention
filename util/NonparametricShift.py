...
class NonparametricShift(object):
    def buildAutoencoder(self, target_img, normalize, interpolate,  nonmask_point_idx, mask_point_idx, patch_size=1, stride=1):
        ...
        #相当于个数 patch的个数
        npatches_part = patches_part.size(0)#完好像素点数量772
        npatches_all = patches_all.size(0)#所有的1024
        npatches_mask=patches_mask.size(0)#损坏的252
    ...
    def _build(self, patch_size, stride, C, target_patches, npatches, normalize, interpolate):
        ...
        conv_enc = nn.Conv2d(C, npatches, kernel_size=patch_size, stride=stride, bias=False)#512-772
        conv_enc.weight.data = enc_patches#为什么把参数变成patch的数值 用于计算相似度？ 有 normalization
        ...
        conv_dec.weight.data = target_patches#没有normalize  1024-512

        return conv_enc, conv_dec

    def _extract_patches(self, img, patch_size, stride, nonmask_point_idx, mask_point_idx):#提取patch
        n_dim = 3
        assert img.dim() == n_dim, 'image must be of dimension 3.'
#kH=1, kW=1，dH, dW=1
        #img 512(8*64通道数) 32 32
        kH, kW = patch_size, patch_size
        dH, dW = stride, stride
        input_windows = img.unfold(1, kH, dH).unfold(2, kW, dW)#将每一个patch（32*32）将每一一个元素单独按行的顺序拿出来排列 得到的shape为（512 32 32 1 1）
        #其中i_1, i_2, i_3, i_4, i_5    ------1：通道数512  2：横轴上的patch个数32   3：纵轴上patch个数 32   4和5分别是patch的长宽 1 1
        ...
        input_windows = input_windows.permute(1,2,0,3,4).contiguous().view(i_2*i_3, i_1, i_4, i_5)#维度换顺序 [:,i,:,:]可以取出第i个通道32*32 为一列的数据
        #[j,:,:,:]取出512个通道上同一位置的数值
