...
def get_norm_layer(norm_type='instance'):
    if norm_type == 'batch':
        norm_layer = functools.partial(nn.BatchNorm2d, affine=True)#包装函数
...
def get_scheduler(optimizer, opt):#scheduler？？
...
def init_weights(net, init_type='normal', gain=0.02): #初始化权重
    def init_func(m):
        classname = m.__class__.__name__
        if hasattr(m, 'weight') and (classname.find('Conv') != -1 or classname.find('Linear') != -1):# 
            if init_type == 'normal':                                                            #初始化的不同方式
...
def init_net(net, init_type='normal', init_gain=0.02, gpu_ids=[]):
    if len(gpu_ids) > 0:
        ...
        net.cuda()
    init_weights(net, init_type, gain=init_gain)            #初始化 权重
    return net
...
def define_G(input_nc, output_nc, ngf, which_model_netG, opt, mask_global, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], init_gain=0.02):
    netG = None
    norm_layer = get_norm_layer(norm_type=norm)#batch normalization layer
    ...
    cosis_list2 = []
    csa_model = []
    if which_model_netG == 'unet_256': #选择哪一种生成器
...
def define_D(input_nc, ndf, which_model_netD,
    ...
    if which_model_netD == 'basic': #选择哪一种判别器
...
class GANLoss(nn.Module):  #loss class
    ...
    def get_target_tensor(self, input, target_is_real):#???
        target_tensor = None                                   #get target tensor   ？？？ create tensor 用来判断 target 是否为空 
        if target_is_real:
            create_label = ((self.real_label_var is None) or
                            (self.real_label_var.numel() != input.numel())) #numel 元素的总个数
            if create_label:
    ...
    def __call__(self, y_pred_fake,y_pred, target_is_real):       # pred——fake 和 pred 的误差？ 和 target_tensor 有点关系
...
class UnetGeneratorCSA(nn.Module):
    def __init__(self, input_nc, output_nc,  num_downs, opt, mask_global, csa_model,cosis_list ,cosis_list2,ngf=64,
        ...
        unet_block = UnetSkipConnectionBlock_3(ngf * 8, ngf * 8, input_nc=None,submodule=unet_block, norm_layer=norm_layer, use_dropout=use_dropout)
        unet_csa = CSA(ngf * 4, ngf * 8, opt, csa_model,cosis_list ,cosis_list2,mask_global, input_nc=None, submodule=unet_block, norm_layer=norm_layer)#加入csa层
...
class UnetSkipConnectionBlock_3(nn.Module): #建立跳跃性链接层 左边变小 右边变大
    def __init__(self, outer_nc, inner_nc,input_nc,
        ...
        if outermost:                                                                  #3种不同的model

            upconv_3 = nn.ConvTranspose2d(inner_nc * 2, outer_nc,                       #outermost 有inner_nc*2
                                                                                       #不懂这些参数的意义
...
class CSA(nn.Module):                                                                                   #csa 层
    def __init__(self, outer_nc, inner_nc, opt,csa_model,cosis_list, cosis_list2,mask_global, input_nc, \
        ...
        csa= CSA_model(opt.threshold, opt.fixed_mask, opt.shift_sz, opt.stride, opt.mask_thred, opt.triple_weight) #加入了csamodel 和innercos
        csa.set_mask(mask_global, 3, opt.threshold)
        csa_model.append(csa)
        innerCos = InnerCos(strength=opt.strength, skip=opt.skip)                                                #innercos ???
...
class UnetGenerator(nn.Module):                                                       #generator
    ...
    def __init__(self, outer_nc, inner_nc, input_nc, 
        ...
        if outermost:
            ...
            upconv = nn.ConvTranspose2d(inner_nc, outer_nc,
                                        ...
                                        padding=1)
            down = [downrelu, downconv]  # for the innermost, no submodule, and delete the bn
            up = [uprelu, upconv, upnorm]# 3个的down 有点不一样
            model = down + up
            # else, the normal
        else:
            upconv = nn.ConvTranspose2d(inner_nc * 2, outer_nc,
...
################################### This is for D ################################### 两个分辨器
# Defines the PatchGAN discriminator with the specified arguments. #一个较低短 一个比较长
...
class PFDiscriminator(nn.Module):
    def __init__(self): #差个norm layer
