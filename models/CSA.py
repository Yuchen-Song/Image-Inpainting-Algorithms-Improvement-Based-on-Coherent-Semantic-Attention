...
class CSA(BaseModel):
    ...
    def initialize(self, opt):
        ...
        self.mask_global.zero_()#mask 里数变为0
        ...
        if len(opt.gpu_ids) > 0:
            self.use_gpu = True
            self.mask_global = self.mask_global.cuda()#net p为粗糙 net G 为精细
        ...
        if self.isTrain:
            ...
            self.criterionGAN = networks.GANLoss(gan_type=opt.gan_type, tensor=self.Tensor) #GAN loss
            ...
            self.optimizers.append(self.optimizer_P)# P rough G refine？ D patch discriminator？ P feature discriminator?
            self.optimizers.append(self.optimizer_D)# D  F都是define D 类型    G P 都是define G 类型
            self.optimizers.append(self.optimizer_F)
            for optimizer in self.optimizers:
                self.schedulers.append(networks.get_scheduler(optimizer, opt))#schedules 中加的是
    ...
    def set_input(self,input,mask):
        ...
        self.input_A.resize_(input_A.size()).copy_(input_A)#复制过来
        self.input_B.resize_(input_B.size()).copy_(input_B)

        self.image_paths = 0

        if self.opt.mask_type == 'center':   #类型如果为 中间 则用 定义好的mask
            self.mask_global=self.mask_global

        elif self.opt.mask_type == 'random':  #类型如果为 任意 则用输入的mask
            self.mask_global.zero_()
            self.mask_global=input_mask
        else:
            raise ValueError("Mask_type [%s] not recognized." % self.opt.mask_type)

        self.ex_mask = self.mask_global.expand(1, 3, self.mask_global.size(2), self.mask_global.size(3)) # 1*c*h*w   mask 扩张为rgb3通道

        self.inv_ex_mask = torch.add(torch.neg(self.ex_mask.float()), 1).byte() #将mask 里0 1 反过来
        self.input_A.narrow(1,0,1).masked_fill_(self.mask_global, 2*123.0/255.0 - 1.0)# 将输入 mask 对应位置填充
        self.input_A.narrow(1,1,1).masked_fill_(self.mask_global, 2*104.0/255.0 - 1.0)#为什么是负数？
    ...
    def set_latent_mask(self, mask_global, layer_to_last, threshold):#设置潜在的mask？
    ...
    def forward(self):
        self.real_A =self.input_A.to(self.device)# 真实的图片 A损坏的图 
        self.fake_P= self.netP(self.real_A)  #net P生成的假的图片 （粗糙）
        self.un=self.fake_P.clone()# 复制假的图片
        self.Unknowregion=self.un.data.masked_fill_(self.inv_ex_mask, 0)#在生成的假的图片中将已经知道的区域填充为0 修补出来未知的区域
        self.knownregion=self.real_A.data.masked_fill_(self.ex_mask, 0)#在原图中将未知的区域填充为0  A中已经知道的区域
        self.Syn=self.Unknowregion+self.knownregion#将 原图中已经知道的图片和生成的未知的区域合起来  第一次修补的粗超结果
        self.CSA_model[0].get_edge(self.Syn,3)  ###增加执行函数
        self.Middle=torch.cat((self.Syn,self.input_A),1)#合成的和 原图 叠在一起
        self.fake_B = self.netG(self.Middle)##增加输入           #net P生成的假的图片（精细） 
        self.real_B = self.input_B.to(self.device)

    def set_gt_latent(self):#应该是得到 通过vgg 网络后的feature map
        gt_latent=self.vgg(Variable(self.input_B,requires_grad=False))
        self.Cosis_list[0].set_target(gt_latent.relu4_3)
        self.Cosis_list2[0].set_target(gt_latent.relu4_3)#设real feature map 为目标


    def test(self):#跟 forward一样
    ...
    def backward_D(self): # 计算两个discriminator 产生的误差 并且反向传导
        fake_AB = self.fake_B
        # Real
        ...
        self.pred_fake = self.netD(fake_AB.detach())#patch discriminator 两个希望被鉴别的误差
        self.pred_real = self.netD(real_AB)
        self.loss_D_fake = self.criterionGAN(self.pred_fake, self.pred_real, True)

        self.pred_fake_F = self.netF(self.gt_latent_fake.relu3_3.detach()) #feature discriminator   （detach 切断vgg反向传导？）
    ...
    def backward_G(self):#计算generator 的误差 并且 向后传导
        ...
        self.loss_G_GAN = self.criterionGAN(pred_fake,pred_real, False)+self.criterionGAN(pred_fake_f, pred_real_F,False)#两个希望蒙混过关的误差

        # Second, G(A) = B
        self.loss_G_L1 =( self.criterionL1(self.fake_B, self.real_B) +self.criterionL1(self.fake_P, self.real_B) )* self.opt.lambda_A# 两个L1 loss


        self.loss_G = self.loss_G_L1 + self.loss_G_GAN * self.opt.gan_weight

        # Third add additional netG contraint loss! consistency loss？
    ...
    def optimize_parameters(self):
        ...
        self.backward_D()            #先对 分辨器 进行更新
        self.optimizer_D.step()
        self.optimizer_F.step()
        self.optimizer_G.zero_grad()
        self.optimizer_P.zero_grad() 
        self.backward_G()      # 对 生成器进行跟新
        self.optimizer_G.step()
        self.optimizer_P.step()


    def get_current_errors(self):     # 得到当前的loss 值
    ...
    def get_current_visuals(self): #得到当前训练成果的图片
    ...
    def save(self, epoch):       #保存 network
